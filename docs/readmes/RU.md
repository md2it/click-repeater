...
Один раз создайте последовательность действий, настройте выполнение и запускайте из окна расширения. Нажатия могут использовать записанные координаты или элементы страницы.
...
- Требуется Firefox 140+ из-за использования расширением свойства манифеста `data_collection_permissions`; Firefox 121–139 не поддерживаются.
- Расширения не работают на системных страницах браузера и защищённых сайтах
- Запись, проверка и выполнение остаются в пределах origin текущего сайта: переход на другой origin останавливает активный сценарий; навигация в пределах того же origin продолжается (повторный inject / resume)
